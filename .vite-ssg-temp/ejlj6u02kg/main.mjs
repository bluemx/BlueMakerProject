...
const supabase$1 = createClient(supabaseUrl, supabaseAnonKey);
...
const useUserStore = defineStore("user", () => {
  ...
  const setUser = async (supabaseAuth) => {
    if (supabaseAuth) {
      ...
      const { data, error, status } = await supabase$1.from("profiles").select("username, role, avatar_url").eq("id", supabaseAuth.user.id).single();
      if (error && status !== 406)
        throw error;
      if (data)
        profile.value = data;
...
async function authFn() {
  supabase$1.auth.getSession().then(({ data }) => {
...
const _sfc_main$l = {
  ...
  setup(__props, { expose: __expose }) {
    ...
    const playuid = {}.VITE_PLAY_USERID;
    const playsk = {}.VITE_PLAY_SK;
    const playOptions = {
      headers: {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "bg-primary-900 px-1 pb-5 border-t-teal border-t-4 mt-2" }, _attrs))}><div class="font-bold text-xs p-1 mt-4">Conversación</div>`);
      _push(ssrRenderComponent(unref(draggable), {
        ...
        item: withCtx(({ element, index }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="bg-slate-600/60 ring-1 text-xs mb-0.5 transition-all"${_scopeId}><div class="flex gap-1 items-center justify-between bg-gradient-to-r from-teal-600 to-primary-300"${_scopeId}><div class="handle items-center justify-between grow px-1 cursor-grab"${_scopeId}>${ssrInterpolate(element.name)}</div>`);
            ...
            _push2(`</div><div${_scopeId}><textarea placeholder="Diálogo" class="w-full resize-none text-xs rounded text-white bg-dark"${_scopeId}>${ssrInterpolate(element.content)}</textarea>`);
            if (element.audiofile) {
              _push2(`<div class="text-xs flex gap-1 pb-2 px-1"${_scopeId}><div class="rounded flex justify-center gap-1"${_scopeId}>`);
              ...
              _push2(`<audio${ssrRenderAttr("src", element.audiofile.audioUrl)} type="audio/mp3" invisible w-0${ssrRenderAttr("id", "dial" + index)}${_scopeId}></audio></div><div class="text-[6px]"${_scopeId}>${ssrInterpolate(element.audiofile.audioUrl)}</div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div>`);
            ...
            return [
              createVNode("div", { class: "bg-slate-600/60 ring-1 text-xs mb-0.5 transition-all" }, [
                createVNode("div", { class: "flex gap-1 items-center justify-between bg-gradient-to-r from-teal-600 to-primary-300" }, [
                  createVNode("div", { class: "handle items-center justify-between grow px-1 cursor-grab" }, toDisplayString(element.name), 1),
                ...
                createVNode("div", null, [
                  withDirectives(createVNode("textarea", {
                    placeholder: "Diálogo",
                    class: "w-full resize-none text-xs rounded text-white bg-dark",
                  ...
                  element.audiofile ? (openBlock(), createBlock("div", {
                    key: 0,
                    class: "text-xs flex gap-1 pb-2 px-1"
                  }, [
                    createVNode("div", { class: "rounded flex justify-center gap-1" }, [
                      createVNode(_component_UButton, {
                        size: "xs",
                        class: "i-solar-play-linear bg-white",
                        onClick: ($event) => playudio("dial" + index)
      ...
      _push(`<div class="bg-slate-400 p-1 mt-2"><!--[-->`);
      ssrRenderList(unref(audiofiles), (item, index) => {
        _push(`<div class="${ssrRenderClass([" w-full  bg-slate-700 mb-0.5 p-1 "])}"><div class="flex gap-1 items-center text-sm"><div><strong class="text-sm">${ssrInterpolate(item.voice)}</strong><span class="text-[8px] leading-2 inline-block ml-1">${ssrInterpolate(item.content)}</span></div><div class="rounded flex justify-center gap-1 ml-auto">`);
        _push(ssrRenderComponent(_component_UButton, {
          size: "xs",
          class: "i-solar-play-linear bg-white",
...
const _sfc_main$g = {
  ...
  setup(__props) {
    const configuration = new Configuration({
      apiKey: window.atob({}.VITE_OPENAI_API_KEY)
    });
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    supabase$1.auth.onAuthStateChange((_, _session) => {
...
const __pages_import_0__ = () => import("./assets/index-77ad83c9.js");
const __pages_import_2__ = () => import("./assets/dashboard-19517fa4.js");
const __pages_import_3__ = () => import("./assets/about-ec73afb8.js");
const __pages_import_4__ = () => import("./assets/README-f009a94f.js");
const __pages_import_5__ = () => import("./assets/_...all_-022ff2c0.js");
const __pages_import_6__ = () => import("./assets/_name_-8e3cd327.js");
const __pages_import_7__ = () => import("./assets/_key_-736e9912.js");
