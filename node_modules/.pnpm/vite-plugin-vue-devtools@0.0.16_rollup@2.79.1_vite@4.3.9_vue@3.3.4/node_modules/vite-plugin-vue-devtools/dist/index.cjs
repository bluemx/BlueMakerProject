...
const fs$8 = require('node:fs/promises');
...
const DIR_DIST = typeof __dirname !== "undefined" ? __dirname : p.dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (document.currentScript && document.currentScript.src || new URL('index.cjs', document.baseURI).href))));
const DIR_CLIENT = p.resolve(DIR_DIST, "../dist/client");
...
const events_1 = require$$0__default$4;
...
async function getStaticAssets(config) {
  ...
  return await Promise.all(files.map(async (path) => {
    ...
    const publicDirname = p__default.relative(config.root, config.publicDir);
    const normalizedPath = publicDirname === p__default.basename(p__default.dirname(path)) ? path.replace(publicDirname, "") : path;
    return {
      path: normalizedPath,
      filePath,
...
function PluginVueDevtools() {
  ...
  function configureServer(server) {
    ...
    server.middlewares.use(`${base}__devtools`, sirv__default(DIR_CLIENT, {
      single: true,
      dev: true
    }));
    createRPCServer("vite-plugin-vue-devtools", server.ws, {
