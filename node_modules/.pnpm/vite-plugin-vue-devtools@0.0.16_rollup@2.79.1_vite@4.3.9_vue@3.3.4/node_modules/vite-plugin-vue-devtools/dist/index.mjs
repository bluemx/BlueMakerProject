...
import p, { dirname, resolve as resolve$1 } from 'node:path';
import { normalizePath } from 'vite';
import sirv from 'sirv';
import Inspect from 'vite-plugin-inspect';
import VueInspector from 'vite-plugin-vue-inspector';
import fs$8 from 'node:fs/promises';
...
async function getStaticAssets(config) {
  ...
  return await Promise.all(files.map(async (path) => {
    ...
    const stat = await fs$8.lstat(filePath);
...
async function getImageMeta(filepath) {
  ...
  try {
    const meta = imageMeta(await fs$8.readFile(filepath));
...
async function getTextAssetContent(filepath, limit = 300) {
  try {
    const content = await fs$8.readFile(filepath, "utf-8");
