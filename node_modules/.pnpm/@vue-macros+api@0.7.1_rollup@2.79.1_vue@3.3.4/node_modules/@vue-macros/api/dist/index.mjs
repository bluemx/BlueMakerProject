...
function genRuntimePropDefinition(types, isProduction, properties) {
  ...
  if (types) {
    ...
    if (hasUnknown)
      types = types.filter((t) => t !== UNKNOWN_TYPE);
    if (isProduction) {
      types = types.filter(
...
var builtInTypesHandlers = {
  Partial: {
    handleType(resolved) {
      var _a;
      return (_a = resolved.typeParameters) == null ? void 0 : _a.params[0];
    },
...
`);
  ...
  async function resolveDefinitions(typeDeclRaw2) {
    ...
    if (builtInTypesHandler == null ? void 0 : builtInTypesHandler.handleTSProperties)
...
`);
  ...
  async function resolveDefinitions(typeDeclRaw2) {
    ...
    for (const signature of properties.callSignatures) {
      ...
      if (isTSNamespace(evtType) || !(evtType == null ? void 0 : evtType.type))
        continue;
