...
var _chunkQIFC2L3Tjs = require('./chunk-QIFC2L3T.js');
...
var PrePlugin = _unplugin.createUnplugin.call(void 0, 
  (userOptions = {}) => {
    ...
    return {
      ...
      enforce: "pre",
      loadInclude(id) {
        return id.includes(_chunkQIFC2L3Tjs.QUERY_TEMPLATE);
      },
      load(id) {
        var _a;
        const { filename, query } = _chunkQIFC2L3Tjs.parseVueRequest.call(void 0, id);
        const content = (_a = templateContent[filename]) == null ? void 0 : _a["mainTemplate" in query ? _chunkQIFC2L3Tjs.MAIN_TEMPLATE : query.name];
        return content;
      },
      transformInclude(id) {
        return filter(id) || id.includes(_chunkQIFC2L3Tjs.QUERY_NAMED_TEMPLATE);
      },
      transform(code, id) {
        if (id.includes(_chunkQIFC2L3Tjs.QUERY_NAMED_TEMPLATE)) {
          const { filename, query } = _chunkQIFC2L3Tjs.parseVueRequest.call(void 0, id);
          const { name: name2 } = query;
          const request = `${filename}?vue&${_chunkQIFC2L3Tjs.QUERY_TEMPLATE}&name=${name2}`;
...
export default {
  render: (...args) => {
    ...
    return typeof r === 'string' ? createTextVNode(r) : r
          ...
          return _chunkQIFC2L3Tjs.preTransform.call(void 0, code, id, templateContent);
        }
      }
    };
  }
...
var PostPlugin = _unplugin.createUnplugin.call(void 0, 
  (userOptions = {}) => {
    ...
    function transformInclude(id) {
      return filter(id) || id.includes(_chunkQIFC2L3Tjs.QUERY_TEMPLATE);
    }
    return {
      name: `${name}-post`,
      enforce: "post",
      transformInclude,
      transform(code, id) {
        return _chunkQIFC2L3Tjs.postTransform.call(void 0, code, id, customBlocks);
      },
      rollup: {
        transform: {
          order: "post",
